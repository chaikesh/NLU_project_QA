...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, RNNEncoder1, RNNEncoder2, maybe_mask_affinity, dcn_decode, maybe_dropout
...
class QAModel(object):
    ...
    def add_placeholders(self):
        ...
        self.document_length = tf.placeholder(tf.int32, (None,), name='paragraph_length')
    ...
    def build_graph(self):
        ...
        affinity_t = maybe_mask_affinity(unmasked_affinity_t, self.question_length)
        ...
        context2 = encoder1.build_graph(summary_d, self.context_mask) # (batch_size, context_len, hidden_size*2)
        question2 = encoder1.build_graph(summary_q, self.qn_mask) # (batch_size, question_len, hidden_size*2)
        
        
        unmasked_affinity1 = tf.einsum('ndh,nqh->ndq', context2, question2)  # [N, D, Q] or [N, 1+D, 1+Q] if sentinel
        affinity1 = maybe_mask_affinity(unmasked_affinity1, self.document_length)
        ...
        affinity_t1 = maybe_mask_affinity(unmasked_affinity_t1, self.question_length)
        ...
        # decoder
        logits = dcn_decode(U, self.document_length, 100, 4, 4, keep_prob=maybe_dropout(self.keep_prob, True))
        last_iter_logit = logits.read(4-1)
        self.logits_start, self.logits_end = last_iter_logit[:,:,0], last_iter_logit[:,:,1]
#        with vs.variable_scope("StartDist"):
#            softmax_layer_start = SimpleSoftmaxLayer()
#            self.logits_start1, self.probdist_start = softmax_layer_start.build_graph(blended_reps_final, self.context_mask)
#
#        # Use softmax layer to compute probability distribution for end location
#        # Note this produces self.logits_end and self.probdist_end, both of which have shape (batch_size, context_len)
#        with vs.variable_scope("EndDist"):
#            softmax_layer_end = SimpleSoftmaxLayer()
#            self.logits_end1, self.probdist_end = softmax_layer_end.build_graph(blended_reps_final, self.context_mask)
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        input_feed[self.question_length] = batch.qn_length
        input_feed[self.document_length] = batch.context_length
